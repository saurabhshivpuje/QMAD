...
function one_step!(A::Ansatz{false}, He, Ha, dt)
    ...
    #cross = 2 * imag(ψ' * (He * Ha - Ha * He) * ψ)
    varHe = 2 * real(ψ' * He * He * ψ - (ψ' * He * ψ)^2)
    #dist = varHe - vmv + bound + cross
    #@debug "McLachlan distance:" dist
    #@debug "Lower bound:":bound
...
function one_step_non_adp!(A::Ansatz, He, Ha, dt)
    ...
    dθ, = lin_solve(M, V)
    #vmv = V' * inverse(M) * V
    #vHa = 2 * real(ψ' * Ha * Ha * ψ + (ψ' * Ha * ψ)^2) #
    #cross = 2 * imag(ψ' * (He * Ha - Ha * He) * ψ)
    #varHe = 2 * real(ψ' * He * He * ψ - (ψ' * He * ψ)^2)
    #dist = varHe - vmv + vHa + cross
    #@show dist
    #@show vHa
    #@show vmv
    #@show cross
